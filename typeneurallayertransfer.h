...
long typeNeuralLayerTransferFeedForward(typeNeuralLayer* parent, typeNeuralLayer* next)
{
	...
	return cudaNeuralArrayTransferForward(&next->layerOutputs, &parent->layerOutputs, (unsigned long)parent->layerValue1);
}
...
long typeNeuralLayerTransferBackPropagate(typeNeuralLayer* parent, typeNeuralLayer* next)
{
	...
	return cudaNeuralArrayTransferReverse(&next->layerDeltas, &parent->layerDeltas, &parent->layerOutputs, (unsigned long)parent->layerValue1);
}
